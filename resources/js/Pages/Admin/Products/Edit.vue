...
import { watch, ref } from 'vue'; // Добавляем ref для управления файлами изображений

const props = defineProps({
    product: Object, // Продукт, который мы редактируем
    categories: Array, // Категории для выпадающего списка
});

// Инициализация формы с данными существующего продукта
const form = useForm({
    _method: 'put', // Важно для методов PUT/PATCH в Laravel
    ...
    new_images: [], // Для новых загружаемых изображений
    deleted_images: [], // Для ID изображений, которые нужно удалить
});

// Состояние для предпросмотра новых изображений
const newImagePreviews = ref([]);

// Состояние для хранения текущих изображений продукта для отображения и удаления
const currentImages = ref(props.product.images || []);

// Автоматическая генерация slug при изменении названия (для удобства)
// Но в режиме редактирования пользователь может захотеть изменить его вручную.
// Можно сделать эту логику более умной, например, только если slug еще не был изменен вручную.
watch(() => form.name, (newName) => {
    // Генерируем slug только если текущий slug пуст ИЛИ если он совпадает с преобразованным старым названием
    // Это предотвратит перезапись вручную введенного slug
...
const handleNewImageUpload = (event) => {
    ...
    newImagePreviews.value = []; // Очищаем старые предпросмотры

    // Создаем предпросмотры для новых изображений
...
const deleteImage = (imageId) => {
    ...
    // Удаляем изображение из списка отображаемых текущих изображений
...
const submit = () => {
    form.post(route('admin.products.update', props.product.id), {
        ...
        onSuccess: () => {
            // После успешного обновления, сброс формы не всегда желателен в режиме редактирования.
            // Вместо этого можно просто перенаправить или обновить данные.
            // router.visit(route('admin.products.index'), {
            //     preserveScroll: true,
            // });
            // Или если вы хотите остаться на странице редактирования,
            // но обновить данные, можно сделать router.reload().
            // В данном случае, redirect()->route() в контроллере уже делает перенаправление.
        },
        onError: (errors) => {
                ...
                <form @submit.prevent="submit">
                    ...
                    <div class="mb-4">
                        ...
                        <input
                            ...
                            required
                        />
                        <div v-if="form.errors.slug" class="text-sm text-red-600 mt-2">
                            {{ form.errors.slug }}
                        </div>
                        <p class="text-xs text-gray-500 mt-1">Отображается в URL (например, /products/название-продукта).</p>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea
                            ...
                            required
                        ></textarea>
                        <div v-if="form.errors.description" class="text-sm text-red-600 mt-2">
                    ...
                    <div class="mb-4">
                        ...
                        <input
                            ...
                            required
                        />
                        <div v-if="form.errors.price" class="text-sm text-red-600 mt-2">
                            {{ form.errors.price }}
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div v-if="form.errors.category_id" class="text-sm text-red-600 mt-2">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea
                            ...
                            rows="3"
                        ></textarea>
                        <div v-if="form.errors.feature" class="text-sm text-red-600 mt-2">
                            {{ form.errors.feature }}
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input
                            ...
                            required
                        />
                        <div v-if="form.errors.quantity" class="text-sm text-red-600 mt-2">
                    ...
                    <div class="mb-4">
                        ...
                        <div v-if="currentImages.length > 0" class="flex flex-wrap gap-4">
                            <div v-for="image in currentImages" :key="image.id" class="relative w-32 h-32 border rounded-md overflow-hidden flex items-center justify-center">
                                <img :src="`/storage/${image.path}`" :alt="product.name" class="max-w-full max-h-full object-contain">
                                <button
                                    type="button"
                    ...
                    <div class="mb-4">
                        ...
                        <input
                            ...
                            accept="image/*"
                        />
                        <div v-if="form.errors.new_images" class="text-sm text-red-600 mt-2">
                            {{ form.errors.new_images }}
                        </div>
                        <template v-if="form.errors['new_images.0']">
                            <div class="text-sm text-red-600 mt-2">
                                {{ form.errors['new_images.0'] }}
                            </div>
                        </template>
